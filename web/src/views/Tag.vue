<template>
  <div class="main-container" v-if="model">
    <div class="main-content archive-page">
      <div class="post-title text-green fs-xxxxl">标签分类的博文--点击自动查找对应Tag</div>
      <div>
        <div class="post-tags">
    ...
    <div class="main-content archive-page">
      <div v-for="item in model" :key="item._id" :id="`${item._id}`" class="post-lists anchor">
        <div>
          <div class="categories-title">{{item._id}} : 共{{item.count}}篇</div>
          <div v-for="item in item.list" :key="item.article._id" class="post-list-item">
            <div class="post-list-item-container bg-postcolor show">
              <div class="item-label">
                ...
                <div class="item-meta">
                  <div
                    class="item-meta-date"
                  >{{item.article.createdAt | date("YYYY-MM-DD HH:mm:ss")}}</div>
                </div>
              </div>
            </div>
          </div>
